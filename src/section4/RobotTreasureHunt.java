...
public class RobotTreasureHunt implements KeyEventDispatcher{
	...
	private void spaceBarWasPressed() {
		...
		int robotXLocation = ethan.getX();
		int robotYLocation = ethan.getY();
		...
		if(robotYLocation == 400 && robotXLocation == 750){
		...
		if(robotYLocation == 100 && robotXLocation == 650) {
			JOptionPane.showMessageDialog(null, "Go to the skull's nostril.");
		}
		
		if(robotYLocation == 100 && robotXLocation == 250) {
			JOptionPane.showMessageDialog(null, "Go to the robot.");
		}
		
		// 9.  If the robot is in the final location
		//     --call the treasureFound() method
		if(robotYLocation == 400 && robotXLocation == 550) {
